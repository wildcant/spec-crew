...
- Workflow states (`prd-draft`, `ready-for-slicing`, `ready-for-review`, `changes-requested`, `review-approved`, `ready-for-builder-pr-merge`, `ready-for-final-pr-approval`, `ready-for-human-merge`, `ready-for-acceptance`, `needs-human-decision`) live in platform state fields.
...
11. Use the issue, Builder PR, and Git/PR APIs as the handoff record. Do not add agent packet schemas to issue text.
12. Assign review-ready issues to Reviewer with the public issue and Builder PR link.
...
- `review_result: approved`: mark `review-approved`. Do not skip the Final PR creation-approval gate.
...
- After the approved Builder PR is merged into `source_branch`, mark `ready-for-final-pr-approval` and ask for explicit authorization to create the Final PR. This authorization is not product acceptance.
- After authorization, first check whether `source_branch` is already reachable from `final_pr_target`. If it is, verify the target environment and mark `ready-for-acceptance`.
- Otherwise create the Final PR:
- Open it on GitHub with `gh pr create`:
  - `--head <source_branch>`
...
- If the Final PR conflicts, mark `needs-human-decision`; otherwise mark `ready-for-human-merge` and provide the Final PR URL. After its merge is verified, mark `ready-for-acceptance`.
...
- If the user reports problems, create a new fix child issue and run the normal Builder → Reviewer → Final PR → acceptance flow. After acceptance, mark the child issue Done.

Post-acceptance:

- Retain `source_branch`. Final PR merge is a human gate.
...
- If Final PR, merge, product acceptance, or other human confirmation remains, mark the parent `ready-for-human-merge` or equivalent and ask the human.
